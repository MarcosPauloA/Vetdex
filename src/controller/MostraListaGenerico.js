...
export default function MostraNomesPatologias(){
  ...
  const { nomeCategoriaEstudo, nomeBuscado } = route.params;
  ...
  // Estado local para armazenar a lista da categoria de estudo genérica
  const [listaGenerica, setListaGenerica] = useState([]); 

  // Função assíncrona para buscar a lista de categoria de patologias da API
  const fetchListaGenerica = async () => {
    try {
        const response = await fetch(`${API_URL}/${nomeCategoriaEstudo}`);
